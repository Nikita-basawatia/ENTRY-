...
//new2
  ...
  useEffect(() => {
    if (!deviceCode) {
      ...
      const randomCode = Math.floor(1000 + Math.floor(Math.random() * 4500) * 2);
  ...
  const handleCodeSubmit = () => {
    ...
    const computedCode = ((userCodeNumber * 2)+6 -46).toString();
  ...
  return (
    <div >
      ...
      <div className='submitbutton'>
        <button className='inputfieldcode' onClick={handleCodeSubmit}>Submit Code</button>
      
      </div>
    <button className = 'do' onClick={handleClearDeviceCode}>do not click - Clear Device Code</button>
